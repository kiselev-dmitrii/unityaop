...
namespace Assets.UnityAOP.Observable.ChainedObservers {
    public abstract class TypedListObserver<T> : BaseChainedObserver, IListObserver<T> {
        private ObservableList<T> observableList; 
    
        protected TypedListObserver(IObservable root, PropertyMetadata[] propertyPath) : base(root, propertyPath) {
            observableList = null;
            Bind(0);
        }
