...
namespace Assets.UnityAOP.Editor.Inspector {
	[CustomPropertyDrawer(typeof(Assets.UnityAOP.Utils.SerializableType))]
	[CustomPropertyDrawer(typeof(ClassConstraintAttribute), true)]
	public sealed class SerializableTypePropertyDrawer : PropertyDrawer {

		#region Type Filtering
		public static Func<ICollection<Type>> ExcludedTypeCollectionGetter { get; set; }

		private static List<Type> GetFilteredTypes(ClassConstraintAttribute filter) {
		...
		private static void FilterTypes(Assembly assembly, ClassConstraintAttribute filter, ICollection<Type> excludedTypes, List<Type> output) {
		...
		private static readonly int s_ControlHint = typeof(SerializableTypePropertyDrawer).GetHashCode();
		private static GUIContent s_TempContent = new GUIContent();

		private static string DrawTypeSelectionControl(Rect position, GUIContent label, string classRef, ClassConstraintAttribute filter) {
		...
		private static void DrawTypeSelectionControl(Rect position, SerializedProperty property, GUIContent label, ClassConstraintAttribute filter) {
			try {
		...
		private static void OnSelectedTypeName(object userData) {
			...
			s_SelectedClassRef = Assets.UnityAOP.Utils.SerializableType.GetClassRef(selectedType);
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
			DrawTypeSelectionControl(position, property.FindPropertyRelative("_classRef"), label, attribute as ClassConstraintAttribute);
		}
